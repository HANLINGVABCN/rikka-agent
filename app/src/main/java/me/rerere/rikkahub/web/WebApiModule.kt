...
fun Application.configureWebApi(
    ...
    // 读 effectiveJwtEnabled 而非 webServerJwtEnabled: 隧道开启时认证是强制的。
    val jwtEnabled = settingsStore.settingsFlow.value.effectiveJwtEnabled
    ...
    routing {
        route("/api") {
            post("/auth/token") {
                ...
                if (!settings.effectiveJwtEnabled) {
